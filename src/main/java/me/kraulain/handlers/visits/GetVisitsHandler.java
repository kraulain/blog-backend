...
public class GetVisitsHandler implements Handler<RoutingContext> {
  ...
  public GetVisitsHandler(MongoClient dbclient){
    this.mongoDAO = new MongoDAO(dbclient);
  }

  @Override
  public void handle(RoutingContext routingContext) {
    LOGGER.debug("get a batch of "+ Collections.Visit +"s {}",
...
//      int pageSize = values.getInteger("pageSize");
    ...
    Future<JsonArray> future = mongoDAO.retrieveBatch(Collections.Visit, query);
    ...
    future.setHandler(result -> {
      if(future.succeeded()){
        response.put("success", Collections.Visit + "s Retrieved");
        response.put("data", future.result());
        routingContext.response().setStatusCode(HttpURLConnection.HTTP_OK);
      }else{
        response.put("error", Collections.Visit + "s Not Retrieved");
